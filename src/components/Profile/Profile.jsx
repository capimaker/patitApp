...
import {
  toggleFollow,
  fetchLoggedUser,
} from "../../service/followers/followSlice";
import {
  Card,
  Avatar,
  Badge,
...
const Profile = () => {
  ...
  const { user: me, isLoading: followLoading } = useSelector(
    (state) => state.follow
  );
  ...
  useEffect(() => {
    if (id && id !== auth._id) {
      ...
      fetch(
        `http://localhost:8080/user/name/${encodeURIComponent(name)}`
      )
        .then((res) => res.json())
  ...
  return (
    <div className="profile-container">
      ...
      <h3 className="profile-posts-title">
        Publicaciones de {profileUser.name}
      </h3>
